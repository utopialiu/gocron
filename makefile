...
.PHONY: gocron
gocron:
	go build  -ldflags "-linkmode external -extldflags -static" $(RACE) -o bin/gocron ./cmd/gocron

.PHONY: node
node:
	go build -ldflags "-linkmode external -extldflags -static" $(RACE) -o bin/gocron-node ./cmd/node
